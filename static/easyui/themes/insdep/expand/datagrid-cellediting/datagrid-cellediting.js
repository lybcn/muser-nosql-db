(function($){
	...
	function getCurrCell(target){
		...
		if (cell.length){
			return {
				index: parseInt(cell.closest('tr.datagrid-row').attr('datagrid-row-datams')),
	...
	function getSelectedCells(target){
		...
		panel.find('td.datagrid-row-selected').each(function(){
			var td = $(this);
			cells.push({
				index: parseInt(td.closest('tr.datagrid-row').attr('datagrid-row-datams')),
				field: td.attr('field')
			});
		});
		return cells;
	...
	function enableCellSelecting(target){
		...
		dc.body1.add(dc.body2).unbind('.cellediting').bind('click.cellediting', function(e){
			...
			if (tr.length && tr.parent().length){
				...
				if (td.length){
					var index = parseInt(tr.attr('datagrid-row-datams'));
	...
	$.extend($.fn.datagrid.methods, {
		...
		cell: function(jq){		// get current cell info {datams,field}
