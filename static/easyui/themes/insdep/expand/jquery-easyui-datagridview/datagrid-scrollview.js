...
$.extend($.fn.datagrid.defaults.finder, {
	getRow: function(target, p){	// p can be row datams or tr object
		var index = (typeof p == 'object') ? p.attr('datagrid-row-datams') : p;
...
var scrollview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	bindEvents: function(target){
		...
		body.unbind('click').bind('click', function(e){
			...
			if (tt.hasClass('datagrid-row-expander')){
				var rowIndex = parseInt(tr.attr('datagrid-row-datams'));
	...
	// insertRow: function(target, datams, row){
	// 	var state = $.data(target, 'datagrid');
	// 	var data = state.data;

	// 	if (datams == undefined || datams == null) datams = data.rows.length;
	// 	if (datams > data.rows.length) datams = data.rows.length;
	// 	$.fn.datagrid.defaults.view.insertRow.call(this, target, datams, row);
	// 	if (data.firstRows && datams <= data.firstRows.length){
	// 		data.firstRows.splice(datams, 0, row);
	// 	}
	// },
	...
	// deleteRow: function(target, datams){
	// 	var data = $(target).datagrid('getData');
	// 	$.fn.datagrid.defaults.view.deleteRow.call(this, target, datams);
	// 	if (data.firstRows){
	// 		data.firstRows.splice(datams, 1);
	// 	}
	// },
...
$.extend($.fn.datagrid.methods, {
	getRowIndex: function(jq, id){
		...
		if (opts.view.type == 'scrollview'){
			// return jq.datagrid('baseGetRowIndex', id) + opts.view.datams;
