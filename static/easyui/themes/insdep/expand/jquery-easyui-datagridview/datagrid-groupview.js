...
var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	renderGroup: function(target, groupIndex, group, frozen){
		...
		table.push('<div group-datams=' + groupIndex + ' ' + cs + '>');
		...
		for(var j=0; j<group.rows.length; j++) {
			...
			table.push('<tr id="' + rowId + '" datagrid-row-datams="' + index + '" ' + cls + ' ' + style + '>');
	...
	bindEvents: function(target){
		...
		body.unbind('click').bind('click', function(e){
			...
			if (expander.length){
				var gindex = expander.closest('div.datagrid-group').attr('group-datams');
...
$.extend($.fn.datagrid.methods, {
    ...
    expandGroup:function(jq, groupIndex){
        return jq.each(function(){
            ...
            var group = view.find(groupIndex!=undefined ? 'div.datagrid-group[group-datams="'+groupIndex+'"]' : 'div.datagrid-group');
    ...
    scrollToGroup: function(jq, groupIndex){
    	return jq.each(function(){
			...
			var dc = state.dc;
			var grow = dc.body2.children('div.datagrid-group[group-datams="'+groupIndex+'"]');
			if (grow.length){
...
$.extend(groupview, {
	refreshGroupTitle: function(target, groupIndex){
		...
		var span = dc.body1.add(dc.body2).children('div.datagrid-group[group-datams=' + groupIndex + ']').find('span.datagrid-group-title');
	...
	resizeGroup: function(target, groupIndex){
		...
		if (groupIndex == undefined){
			...
			var groupHeader = dc.body2.children('div.datagrid-group[group-datams=' + groupIndex + ']');
		}
	...
	updateRow: function(target, index, row){
		...
		var groupIndex = parseInt(tb.prev().attr('group-datams'));
		this.refreshGroupTitle(target, groupIndex);
	},
	...
	deleteRow: function(target, index){
		...
		if (group.rows.length > 1){
			...
			body.children('div.datagrid-group[group-datams='+groupIndex+']').remove();
			for(var i=groupIndex+1; i<this.groups.length; i++){
				body.children('div.datagrid-group[group-datams='+i+']').attr('group-datams', i-1);
			}
